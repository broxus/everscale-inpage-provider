...
class ProviderProxy implements Provider {
  ...
  async request<T extends ProviderMethod>(data: RawProviderRequest<T>): Promise<RawProviderApiResponse<T>> {
    if (this.inner == null) {
...
class Connector {
  ...
  getKeyByProvider(provider: Provider): string | undefined {
    const wallet = this.params.supportedWallets.find(w => (window as any)[w.injected.object] === provider);
    if (wallet) {
      return wallet.title;
    }
