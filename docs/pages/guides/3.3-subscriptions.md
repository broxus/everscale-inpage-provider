...
In the TON-compatible blockchains, subscribing to events and messages is a crucial aspect because of the nature of how messages are handled and delivered. In particular, the delivery of deferred messages (messages that are not processed immediately) can be delayed, especially when they are sent to accounts in different shards. To handle these cases effectively and to ensure that your application receives updates in real-time, it is necessary to subscribe to the relevant events and messages.
...
const example = new provider.Contract(exampleAddress, exampleAbi);
...
The `provider.Subscriber` class is used to manage the subscription to contract events. It allows you to easily subscribe and unsubscribe from events related to a specific contract instance.

Using a `Contract` instance also provides methods like `waitForEvent` and `getPastEvents` to handle transaction events, decode contract events, and wait for events that match specific criteria.

```typescript
// Wait for a specific event
const event = await example.waitForEvent({
  filter: 'StateChanged',
});
